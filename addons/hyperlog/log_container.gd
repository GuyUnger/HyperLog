	...
	if parent_node and parent_node != get_tree().get_root():
		if parent_node is Node3D:
			if HyperLog.camera_3d != null:
				position = HyperLog.camera_3d.unproject_position(parent_node.global_position) + _offset
			else:
				position = _offset
		else:
			position = parent_node.get_global_transform_with_canvas().origin + _offset
		
		if align_horizontal == HORIZONTAL_ALIGNMENT_CENTER:
			position.x -= size.x / 2 * scale.x
		elif align_horizontal == HORIZONTAL_ALIGNMENT_RIGHT:
			position.x -= size.x * scale.x
		if align_vertical == VERTICAL_ALIGNMENT_CENTER:
			position.y -= size.y / 2 * scale.y
		elif align_vertical == VERTICAL_ALIGNMENT_BOTTOM:
